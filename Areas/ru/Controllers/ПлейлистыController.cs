...
{
    public class ПлейлистыController : Controller
    {
        //
        // GET: /ru/Плейлисты/
        ...
        public ActionResult AddToPlayList()
        {
            ...
            if (User.Identity.IsAuthenticated == false)
                return Content("<script type='text/javascript'>window.location='/ru/Аккаунт/Войти';</script>");
            //--------------------------------------------------------------------------------------------
        ...
        public ActionResult SubmitAddNewSong(string Song_Title, string Song_Guid, int? Sel_Playlist_ID, int? StayHereCheck, string ss)
        {
            ...
            if (StayHereCheck != null && StayHereCheck == 1)
            {
                if (ss != null && ss.ToString() != "")
                    return RedirectPermanent("/ru/Слушать?ss=" + ss.Replace(' ', '+'));
                else
                    return RedirectPermanent("/ru/Создать/Плейлист");
            }
            else
            {
                return RedirectPermanent("/ru/Создать/Плейлист?playlist_id=" + Sel_Playlist_ID);
            }
            //-----------------------------------------------------------------------------------------
        ...
        public ActionResult getPlaylistDetailsEdt()
        {
            ...
            //--------------------------------------------------------------------------------------------
            if (User.Identity.IsAuthenticated == false)
                return RedirectPermanent("/ru/Аккаунт/Войти");
            //--------------------------------------------------------------------------------------------
