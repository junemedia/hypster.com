...
namespace hypster.Controllers
{
    public class playlistsController : Controller
    {
        ...
        public ActionResult artist(string id)
        {
            ...
            searchListRequest.Q = HttpUtility.UrlEncode(id.Replace("+", " ")); // Replace with your search term.
            ...
            foreach (var searchResult in searchListResponse.Items)
            {
                switch (searchResult.Id.Kind)
                {
                    case "youtube#video":
                        ...
                        song_add1.YoutubeId = searchResult.Id.VideoId;
                        song_add1.Title = searchResult.Snippet.Title;
        ...
        public ActionResult station(string id)
        {
            ...
            if (__genre_found == false)
            {
                ...
                var searchListRequest = youtubeService.Search.List("snippet");
                searchListRequest.Q = Genre_str; // Replace with your search term.
                searchListRequest.MaxResults = 50;
        ...
        public string ListenHistory()
        {
            ...
            hypster_tv_DAL.listenHistoryManagement histManager = new hypster_tv_DAL.listenHistoryManagement();
            hypster_tv_DAL.memberManagement memberManager = new hypster_tv_DAL.memberManagement();
            ...
            {
                curr_user_id = memberManager.getMemberByUserName(User.Identity.Name).id;
            }

            if (Request.QueryString["title"] != null)
            {
                curr_title = Request.QueryString["title"].ToString();
            }

            if (Request.QueryString["url"] != null)
            {
                curr_url = Request.QueryString["url"].ToString();
            }
            //---------------------------------------------------------------------
            ...
            //---------------------------------------------------------------------
            if (curr_user_id > 0 && curr_url != "")
            {
                hypster_tv_DAL.ListenHistory hist = new hypster_tv_DAL.ListenHistory();
                hist.ListenHistory_Date = DateTime.Now;
                hist.ListenHistory_GUID = "";
                hist.ListenHistory_Title = curr_title;
                hist.ListenHistory_URL = curr_url;
                hist.ListenHistory_UserID = curr_user_id;

                histManager.AddListenHistory(hist);
            }
            //---------------------------------------------------------------------
        ...
        public ActionResult GetUserHistory()
        {
            ...
            List<hypster_tv_DAL.ListenHistory> hist_list = new List<hypster_tv_DAL.ListenHistory>();
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                hist_list = histManager.GetLatestListenHistoryUser(curr_user_id);
            }


            return View(hist_list);
        }
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
