...
function GeneratePlayerControls() {
    ...
    $("#hypPlayerCont").append("<div id='" + PlayerIDVar + "' style='float:left; width:" + (PL_W-4) + "px; height:" + PL_H + "px; margin:2px 0 0 2px;' ></div>");
    //-------------------------------------------------------------------------------
    ...
    if (hypVisibility == "hidden") {
        $(tag_player_cont).css("height", "50px");

        $("#" + PlayerIDVar).css("width", "1px");
        $("#" + PlayerIDVar).css("height", "1px");
        $("#" + PlayerIDVar).css("filter", "progid:DXImageTransform.Microsoft.Alpha(opacity=10)");
        $("#" + PlayerIDVar).css("opacity", "0.10");
        $("#" + PlayerIDVar).css("-moz-opacity", "0.10");
    }
...
function GenerateControlsBar() {
    ...
    //hypVisibility
    if (hypVisibility == "hidden") {
        $("#hypPlayerCont").append("<div id='hypPlayerMenu' style='float:left; width:" + PL_W + "px; height:27px; margin-top:-3px;' />");
    }
    else {
        $("#hypPlayerCont").append("<div id='hypPlayerMenu' style='float:left; width:" + PL_W + "px; height:27px;' />");
    }



    var btn_style = "float:left; width:" + BTN_W + "px; height:50px; line-height:50px; margin:0 0 0 0; text-align:center; font-size:16px; color:#FFFFFF; cursor:pointer; font-weight:bold;";
    var btn_style_last = "float:left; width:" + BTN_W + "px; height:50px; line-height:50px; margin:0 0 0 0; text-align:center; font-size:16px; color:#FFFFFF; cursor:pointer; font-weight:bold;";



    $("#hypPlayerMenu").append("<a class=\"hypPlayer_PlPs_btn\" href=\"javascript:PlayPauseVideo();\" style=\" " + btn_style + " background:" + hypPlayPauseButtonColor + " url('http://" + hypHostName + "/a/" + imgs_f + "/play_icn.png') no-repeat center;\" ></a>");
    $("#hypPlayerMenu").append("<a class=\"hypPlayer_Prev_btn\" href=\"javascript:PlayPrevVideo();\" style=\" " + btn_style + " background:" + hypPrevButtonColor + " url('http://" + hypHostName + "/a/" + imgs_f + "/prev_icn.png') no-repeat center;\" ></a>");
    $("#hypPlayerMenu").append("<a class=\"hypPlayer_Next_btn\" href=\"javascript:PlayNextVideo();\" style=\" " + btn_style + " background:" + hypNextButtonColor + " url('http://" + hypHostName + "/a/" + imgs_f + "/next_icn.png') no-repeat center;\" ></a>");
    $("#hypPlayerMenu").append("<a class=\"hypPlayer_Shuffle_btn\" href=\"javascript:ShuffleSongs();\" style=\" " + btn_style + " background:" + hypShuffleButtonColor + " url('http://" + hypHostName + "/a/" + imgs_f + "/shuffle_icn.png') no-repeat center;\" ></a>");
    $("#hypPlayerMenu").append("<a class=\"hypPlayer_Hypster_btn\" href=\"http://hypster.com/?rtg=p12\" title=\"Create Free Playlist Online\" target=\"_blank\" style=\" display:block; " + btn_style_last + " background:" + hypHypsterButtonColor + " url('http://" + hypHostName + "/a/" + imgs_f + "/hypster_icn.png') no-repeat center;\" ></a>");
...
function onPlayerReady(event) {
    //alert(" playlistLoaded->" + playlistLoaded);
...
function StartPlayVideo() {
    ...
    player.stopVideo();
    //alert("StartPlayVideo " + items_arr[CURR_VIDEO]);
...
function PlayNextVideo() {
    ...
    player.stopVideo();
    //alert("PlayNextVideo " + items_arr[CURR_VIDEO]);
    player.loadVideoById(items_arr[CURR_VIDEO], 0, 'default');
...
ga('stageTracker.set', 'dimension5', 'HypFM');
