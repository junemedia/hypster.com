...
function PlayPrevVideoN() {
    ...
    if (url_str.indexOf("cs=") > 0) {
        window.location = url_str.replace("cs=" + getQuerystring('cs'), "cs=" + CURR_VIDEO);
        return;
    }

    if (url_str.indexOf("?") > 0) {
        window.location = url_str + "&cs=" + CURR_VIDEO;
        return;
    }
    else {
        window.location = url_str + "?cs=" + CURR_VIDEO;
...
function Reps(a) {
    ...
    try {
        if (curr_user_id == "5042900") {
            $.ajax({
                type: "POST",
                url: "/home/reps/" + "ANRB" + "?a=" + a + "&sid=" + $("#si_" + items_arr[CURR_VIDEO]).val(),
                async: true,
                success: function (data) {
                }
            });
