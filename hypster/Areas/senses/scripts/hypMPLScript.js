    ...
    if (PLAY_STATE == "0") { //IF NOT STARTED START
        ...
        if (iOS == true && readCookie("hypAPPind") == "hypAPPind=1") {
            try {
                ...
                var tag_iOS_ex = document.getElementById('iOS_ex');
                tag_iOS_ex.src = "https://hypster.com/senses/sAccount/SetMusicPlay?playMusic=1";
            }
            catch (ex) { }
        }
    }
...
function Higlight_Item(activeNum) {
    ...
    $("#PlItem_" + activeNum).css("background-color", "#318E8A");


    try {
        CURR_ACTIVE_MENU = activeNum;
    } catch (ex) { }
