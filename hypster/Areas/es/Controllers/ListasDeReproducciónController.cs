...
{
    public class ListasDeReproducciónController : Controller
    {
        //
        // GET: /es/ListasDeReproducción/
        ...
        public ActionResult AddToPlayList()
        {
            ...
            if (User.Identity.IsAuthenticated == false)
                return Content("<script type='text/javascript'>window.location='/es/Cuenta/Registrarse';</script>");
            //--------------------------------------------------------------------------------------------
        ...
        public ActionResult SubmitAddNewSong(string Song_Title, string Song_Guid, int? Sel_Playlist_ID, int? StayHereCheck, string ss)
        {
            ...
            if (StayHereCheck != null && StayHereCheck == 1)
            {
                if (ss != null && ss.ToString() != "")
                    return RedirectPermanent("/es/Escucha?ss=" + ss.Replace(' ', '+'));
                else
                    return RedirectPermanent("/es/Crear/playlist");
            }
            else
            {
                return RedirectPermanent("/es/Crear/playlist?playlist_id=" + Sel_Playlist_ID);
            }
            //-----------------------------------------------------------------------------------------
        ...
        public string applyOrder()
        {
            ...
            if (params_parsed == true)
            {
                ...
                {
                    hypster_tv_DAL.PlaylistData_Song ii_item = songs_list[0];
        ...
        public ActionResult getPlaylistDetailsEdt()
        {
            ...
            //--------------------------------------------------------------------------------------------
            if (User.Identity.IsAuthenticated == false)
                return RedirectPermanent("/es/Cuenta/Registrarse");
            //--------------------------------------------------------------------------------------------
