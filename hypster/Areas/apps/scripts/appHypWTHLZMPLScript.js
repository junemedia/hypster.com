...
function onPlayerStateChange(event) {
    ...
    if (event.data == YT.PlayerState.PAUSED) {
        PLAY_STATE = "0";
        $("#PlPs_btn").css("background-image", "url('/imgs/wthlz/play_icn.jpg')");
    }
    if (event.data == YT.PlayerState.PLAYING) {
        PLAY_STATE = "1";
        $("#PlPs_btn").css("background-image", "url('/imgs/wthlz/pause_icn.jpg')");
    }
}
...
function PlayPauseVideo() {
    ...
    if (PLAY_STATE == "0") { //IF NOT STARTED START
        ...
        if (iOS == true && readCookie("hypAPPind") == "hypAPPind=1") {
            try
            {
                console.log('pauseMusic://param=0');
                //need to det iph
        ...
        if (iOS == true && readCookie("hypAPPind") == "hypAPPind=1") {
            try
            {
                console.log('playMusic://param=1');
                //need to det iph
                window.location.href = "playMusic://param=1";
            }
            catch (ex) { }
        }
    }
...
function Higlight_Item(activeNum) {
    $(".playlistItemC").css("background-color", "#FFFFFF");
    $("#PlItem_" + activeNum).css("background-color", "#e80389");
...
function MuteSound() {
    if (SOUND_STATE == 1) {
        ...
        $("#Mute_btn").css("background-image", "url('/imgs/wthlz/unmute_icn.jpg')");
    }
    else {
        player.unMute();
        SOUND_STATE = 1;

        $("#Mute_btn").css("background-image", "url('/imgs/wthlz/mute_icn.jpg')");
    }
...
function SoundMouseOver() {
    $("#Mute_btn_cont").css("width", "86px");
}

function SoundMouseOut() {
    $("#Mute_btn_cont").css("width", "39px");
}
